<template>
  <label for="version" class="hidden lg:flex items-center">
    <select name="version" id="version" v-model="selectedVersion" class="appearance-none text-gray bg-transparent">
      <option v-for="version in versions" :key="version.text" :value="version">{{ version.text }}</option>
    </select>
    <svg class="ml-2 fill-current h-4 w-4 text-gray" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
