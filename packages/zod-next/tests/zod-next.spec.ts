...
import { toTypedSchema } from '@/zod-next';
...
test('validates typed field with zod', async () => {
  const wrapper = mountWithHoc({
    setup() {
      const rules = toTypedSchema(z.string().min(1, { error: REQUIRED_MSG }).min(8, { error: MIN_MSG }));
...
test('handles zod union errors', async () => {
  ...
  setValue(name, '4');
  await flushPromises();
  expect(nameError.textContent).toBe('Invalid input: expected undefined, received string');
