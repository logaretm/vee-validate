...
const { value, errorMessage } = useField('email', toTypedSchema(z.string().email().required()));
```
...
const { value, errorMessage } = useField('email', required);
```
...
You can also use different prop names for the `modelValue`, for example `v-model:text` can be implemented by passing the model name directly to `syncModel`.
