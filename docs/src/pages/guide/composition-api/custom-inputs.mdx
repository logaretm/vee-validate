...
So far we've been using `useForm` to create forms and use `defineInputBinds` or `defineComponentBinds` to create field-binding objects to integrate them with our fields. However, that usually requires a lot of boilerplate code to create the binding object and the field component.

For example, this is how a 5-field form looks like with `useForm` and `defineInputBinds`:
