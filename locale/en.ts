...
const messages: { [k: string]: ValidationMessageGenerator } = {
  ...
  after: (field, { inclusion }: any, { targetName }: any) =>
    `The ${field} must be after ${inclusion ? 'or equal to ' : ''}${targetName}.`,
  ...
  before: (field, { inclusion }: any, { targetName }: any) =>
    `The ${field} must be before ${inclusion ? 'or equal to ' : ''}${targetName}.`,
  ...
  max: (field, params) => `The ${field} field may not be greater than ${params && params.length} characters.`,
  ...
  required_if: (field, _, { targetName }) =>
    `The ${field} field is required when the ${targetName} field has this value.`,
