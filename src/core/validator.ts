...
import { PartialI18nDictionary } from './i18n';
...
export default class Validator {
  ...
  */
  ...
  static localize(rootDictionary: any): void;
  static localize(lang: any, langDictionary: PartialI18nDictionary): void;


  /**
   * Adds and sets the current locale for the validator.
   */
  static localize (lang: any, dictionary?: any) {
    if (isObject(lang)) {
  ...
  static _guardExtend (name: string, validator: ValidationRule) {
    ...
    if (isCallable((validator as ValidationRuleSchema).validate)) {
      return;
    }
  ...
  _validate (field: any, value: any, { initial = false } = {}) {
    ...
    requireRules.forEach(rule => {
      ...
      if (isPromise(result)) { throw createError('Require rules cannot be async'); }
